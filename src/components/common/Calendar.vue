    ...
    <transition name="fade">
      ...
      <div id="calendar" v-show="changeShowCalendar">
        <div id="calendar-header">
          <span class="arrow" @click="subMonth">&lt</span>
          <span id="date-box">
            {{trueSelectYear}}年{{trueSelectMonth}}月
          </span>
          <span class="arrow" @click="addMonth">&gt</span>
        </div>
        <div class="week">
...
export default {
  ...
  computed: {
    ...
    selectValue: function () {
      let selectMonth = this.trueSelectMonth < 10 ? `0${this.trueSelectMonth}` : `${this.trueSelectMonth}`
      let selectDay = this.trueSelectDay < 10 ? `0${this.trueSelectDay}` : `${this.trueSelectDay}`

      return `${this.trueSelectYear}-${selectMonth}-${selectDay}`
    },
...
@import '../../stylesheets/common/calender.scss'
</style>